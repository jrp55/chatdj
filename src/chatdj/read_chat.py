...
SPOTIFY_URL_PATTERN = \
r"https://open\.spotify\.com/track/(?P<TRACK_ID>[^?\s]+)(\?(?P<EXTRA_PARAMS>[^\s]+))?"


def detect_spotify_tracks(chat_input: str) ->\
        Generator[SpotifyTrack, None, None]:
    for spotify_match in re.finditer(SPOTIFY_URL_PATTERN, chat_input):
        yield SpotifyTrack(spotify_match.group('TRACK_ID'),
                           spotify_match.group('EXTRA_PARAMS'))
