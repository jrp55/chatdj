...
def create_playlist():
    """
    Either renders the input form for GET request, for processes the form input
    on POST request.
    """
    if request.method == 'POST':
        ...
        spotify_client.\
            obtain_access_token_from_spotify(request.form['access_code'])
        file_data = b'\n'.join(request.files['playlist_chat']
                               .stream.readlines()).decode('utf-8')
        if not RATE_LIMITER.register_usage(spotify_client.user_id,
                                           len(file_data)):
            app.logger.info(f'Rejecting input from user\
                    {spotify_client.user_id} as they have breached the fair\
                    usage limit')
            return 'Perhaps try being a bit less greedy?', 429

        app.logger.info(f'User {spotify_client.user_id} passed rate limit\
                checks')
        playlist_input = PlaylistInput.from_flask_request(file_data,
                                                          request,
                                                          app.logger)
        app.logger.info('Created playlist input')

        try:
            playlist_url = spotify_client.\
                    create_playlist_on_spotify(playlist_input)
